...
function coordinate(kei){
  ...
  this.setXY = function(x,y){
    ...
    // 実際の計算実行部分
    xip=xi=(x+m0*Merid(2*phi0[kei]*3600*s2r))/ra ; etap=eta=y/ra ; sgmp=1 ; taup=0
      for(j=beta.length; --j; ) {
...
//    phideg=Math.floor(Lat/10000) ;
    phideg=Math.floor(Lat) ;
    alert(phideg);
//    phimin=Math.floor((Lat-phideg*10000)/100)
    phiminOrg=(Lat-phideg)*60;
    phimin=Math.floor(phiminOrg);
    alert(phimin);
    phisec=(phiminOrg-phimin)*60
    alert(phisec);
    
    ddd = phideg*10000-phimin*100 + phisec;
    
//    phirad=(phideg*3600+phimin*60+Lat-phideg*10000-phimin*100)*s2r
    phirad1=(phideg*3600+phimin*60+ddd-phideg*10000-phimin*100)*s2r
    alert(phirad1);
    phirad = Lat * Math.PI / 180 ;
    alert(phirad);

//    lmbddeg=Math.floor(Lon/10000) ;
    lmbddeg=Math.floor(Lon) ;
    lmbdminOrg=(Lon-lmbddeg)*60
    lmbdmin=Math.floor(lmbdminOrg)
//    lmbdsec=lmbddeg*3600+lmbdmin*60+Lon-lmbddeg*10000-lmbdmin*100
    lmbdsec=(lmbdminOrg-lmbdmin)*60
    
    // 実際の計算実行部分
    sphi=Math.sin(phirad) ;
    nphi=(1-n)/(1+n)*Math.tan(phirad)
    dlmbd=(lmbdsec-lmbd0[kei]*60)*s2r
    sdlmbd=Math.sin(dlmbd) ;
    cdlmbd=Math.cos(dlmbd)
    tchi=sinh(atanh(sphi)-e2n*atanh(e2n*sphi)) ;
    cchi=Math.sqrt(1+tchi*tchi)
    xi=xip=Math.atan2(tchi, cdlmbd) ;
    eta=etap=atanh(sdlmbd/cchi) ;
    sgm=1 ;
    tau=0
    for(j=alp.length; --j; ) {
    ...
    x=ra*xi-m0*Merid(2*phi0[kei]*3600*s2r) ;
    y=ra*eta
    ...
    gbyou=gmm/s2r-gdo*3600-gfun*60  
    // 結果表示
    document.write("<h2>座標系番号： " + kei + "  緯度： " + Lat + "  経度： " + Lon + "<br/><br/>")
